...
describe("admin auth: forgeable headers are not trusted", () => {
  ...
  it("ADMIN_DEV_BYPASS must be exactly 'true' (no other value grants admin)", async () => {
    for (const v of ["false", "yes", "1", undefined, "TRUE", " true "]) {
  ...
  it("x-admin-email IS honored when ADMIN_DEV_BYPASS=true (dev-only bypass)", async () => {
    const email = `dev@local.test`;
    const env = {
      ADMIN_EMAILS: `${ADMIN_EMAIL},${email}`,
  ...
  it("x-admin-email dev bypass still requires the email to be allow-listed", async () => {
    const env = { ADMIN_EMAILS: ADMIN_EMAIL, ADMIN_DEV_BYPASS: "true" };
    const res = await call(
      "/api/admin/users",
      { headers: { "x-admin-email": "not-allowlisted@attacker.com" } },
      env,
    );
    assert.equal(res.status, 401);
  });
