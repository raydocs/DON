...
// source.
//
// `x-admin-email` is a local development/testing bypass, gated behind
// `ADMIN_DEV_BYPASS === "true"` (which must never be set in `wrangler.toml`).
async function getCfAccessUser(c: any): Promise<{ email: string } | null> {
  ...
  // 2. Local development bypass. Off by default and must never be enabled in
  //    production (not present in wrangler.toml [vars]).
  if (!email && c.env.ADMIN_DEV_BYPASS === "true") {
    email =
      (c.req.header("x-admin-email")?.trim().toLowerCase() as string | null) ||
      null;
  }
