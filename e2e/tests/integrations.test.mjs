...
test("authenticated REST API serves its complete OpenAPI contract and CRUD lifecycle", async () => {
  await withApp("integrations-rest", async (app) => {
    ...
    // Stay only just above the limit. Sending a whole extra MiB lets Axum
    // reject while Undici is still writing, which can surface as ECONNRESET
    // instead of exposing the already-produced 413 response to this test.
    const oversizedGroupBody = { name: "G".repeat(2 * 1024 * 1024) };
    assert.ok(
...
test("offline cloud, update, team-lock, and synchronizer contracts are deterministic", async () => {
  await withApp(
    ...
    },
    {
      ...
      extraEnv: { WAYFERN_TEST_TOKEN: "donut-e2e-contracts" },
    },
  );
});
