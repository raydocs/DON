 ...
 * import dialog offers first and the one vendors overwhelmingly ship. A form
 * can supply its selected type for lines without a scheme; explicit schemes
 * always win.
 */
export function pickParsedProxy(
  results: ProxyParseResult[],
  unschemedProxyType = DEFAULT_PROXY_TYPE,
): ParsedProxyLine | null {
  for (const result of results) {
    if (result.status === "parsed") {
      return {
        proxy_type: result.original_line.includes("://")
          ? result.proxy_type
          : unschemedProxyType,
        host: result.host,
        port: result.port,
    ...
    if (result.status === "ambiguous") {
      ...
      if (resolved) {
        return resolved.original_line.includes("://")
          ? resolved
          : { ...resolved, proxy_type: unschemedProxyType };
      }
    }
  }
  return null;
